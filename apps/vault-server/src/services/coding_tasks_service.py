    ...
    def assemble_brief(
        ...
        *,
        ...
        return (
            ...
            "- The Python environment is **already installed system-wide** in this\n"
            "  container. Run `python3 -m pytest tests/` directly from\n"
            "  `apps/vault-server`. Do **not** create a virtualenv: CLAUDE.md's\n"
            "  quick-commands assume a host checkout, and `python3 -m venv` here\n"
            "  produces an empty environment that re-downloads every dependency.\n"
            "  If you somehow must build one anyway, use `uv venv` and `uv pip\n"
            "  install`, never plain `pip`: pip's resolver backtracks for tens of\n"
            "  minutes on this dependency set and produces no output while doing it.\n"
