...
- `GET /daily` - Time-based feed: `{date, tokens_today, tokens_total, schedule[], todos[], notes[]}` — schedule sorted by start time, sources: calendar (filtered by `GOOGLE_CALENDAR_INCLUDE`, default `Mazkir` only), daily-task (timed checkboxes), habit (habits with `scheduled_at`); `todos[]` is every non-moved checkbox in the note, any section
