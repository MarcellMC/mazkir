...
- **Parallel tool execution (P5):** Each tool entry carries `safe_for_parallel: bool`. Read tools default safe; file-tier writes touching distinct paths (`create/update/archive/delete` on task/habit/goal + `save_knowledge`) are overridden to safe; daily-section writes (`daily_*`, `attach_to_daily`, `edit_daily_section`) and event writes stay unsafe. The agent loop dispatches a batch of auto-execute tool calls via `parallel_executor.execute_calls_maybe_parallel` — concurrent via `asyncio.gather` in a worker thread when all calls are safe, serial fallback otherwise.
- **Completing is not a distinct-path write (2026-09-12):** `complete_task` and `complete_habit` were in `SAFE_WRITES` on the premise that a file-tier write touches only its own resolved path. They also award tokens, and that read-modify-writes two files *every* completion shares — `00-system/motivation-tokens.md` and today's daily note. Dispatched in parallel, three completions took a 50-token ledger to **5** while all three tool results reported `tokens_earned: 5`: each thread read a total the others had not written yet, and `write_file`'s `open(path, 'w')` truncate-then-write meant one sometimes read the ledger mid-write as *empty*, so `total_tokens` defaulted to 0. That is the "logged a completion, no tokens" report. Both halves are fixed: the completion tools are serial again (the cost is the bulk-completion latency parallelism bought — May 21 trace: 13.7 s for 11 calls), and `update_tokens` holds `VaultService.mutation_lock` across the whole award so the arithmetic survives concurrent callers from any entry point.
- **`write_file` is atomic (2026-09-12):** it renders to bytes, writes a `.<file>.…tmp` scratch file in the destination directory, and `os.replace`s it in, so a reader sees the old file or the new one and never a torn one. This is the other half of the same bug report: `frontmatter` parses both an empty and an unterminated-YAML file as `metadata == {}` rather than raising, and `list_active_habits` filters on `status == 'active'` — so a habit read mid-write was **silently dropped from the list** while `create_habit` had returned `ok: true`. The temp name is dotted and `.tmp`-suffixed so it matches neither `list_files`' `*.md` glob nor Obsidian's index. `list_active_habits` now also logs a WARNING when a note has no readable frontmatter at all, since `status: inactive` is a decision and unparseable YAML is a fault, and only the first should be silent.
...
- **A reminder is a start with no end, and that must still reach Google (2026-09-12):** `create_event` and `update_event` both gated the calendar sync on `is_complete` — start *and* end — so "add a calendar reminder to give the next Milpro pill in a month" wrote a row with `source_ids: {}` and `calendar_sync: {attempted: false, reason: "incomplete"}` and never reached Google, while the agent replied "Done! 📅 Reminder created". Verified against the Google API a month later: the Mazkir calendar held only a Dog Walk that day. The gate was also refusing a call that would have worked — `calendar_service._build_event` has always defaulted a missing end to `start + default_event_duration`. Both gates now skip only when there is no **start** (`reason: "no_start_time"`), which is the one shape Google cannot be told about: an event known only by when it ended. `crosses_midnight` is guarded by `is_complete` at those call sites, since it can now be reached with a null end.
- **The agent chooses the span and the alerts, not the code (2026-09-12):** `_build_event` imposed 30 minutes and a single 10-minute popup on everything Mazkir created, while the user's own hand-made pill reminders were 5-minute events with popups a day and an hour ahead. `create_event` now forwards `duration_minutes` and a new `remind_minutes_before: int[]` to `CalendarService.create_event`, and the tool schema tells the agent to pick both from the occasion (a pill ~5 min with one short alert; anything needing travel two alerts, `[1440, 60]`). `remind_minutes_before=None` means "no opinion" and keeps the 10-minute default, while `[]` is a decision meaning no alerts — the distinction is `is None`, not falsiness, and a test pins it. The tool description also tells the agent to read `calendar_sync` before claiming a sync, which is the half of the Milpro bug that made it invisible for a month. Note `memory/00-system/skills/time-management.md:48` still says "Told only one, create the block anyway", which is right for logging the past and works against this for a scheduled reminder — a vault edit, so it is not made here.
...
- **The agent reads the user's clock, not the process's (2026-09-13):** `"Current date/time"` in the prompt tail was `datetime.now()` — naive, so the *server's* timezone, which in a container is UTC unless someone set TZ — while `/day`, `MemoryService` and `VaultService` all resolved "now" against `VAULT_TIMEZONE`. Two different failures came out of that one line, both reported as "the event I just made is not in /day". All day it is an offset: the model was told 06:34 while the user's phone said 09:34, so blocks it anchored on "now" (`Reading 06:35–07:35`, `Shower 07:35–08:05`) landed three hours from the day being lived. From 21:00 local until midnight it is a wrong **date**: `dt.date.today()` still says yesterday, so `create_event`'s default date, `_save_photo`'s media directory and every daily-tier handler wrote to yesterday, where today's `/day` cannot show them. `services/clock.py` (`vault_now` / `vault_today` / `vault_today_iso`) is now the only clock the agent path reads, and the prompt line names the zone so the number cannot be re-read as UTC. It takes the zone from `settings` per call rather than from `self.vault.tz`, because a handler with no vault to hand — and a test with a mocked one — still has to get the real answer.
- **An offset-bearing timestamp is converted at the write boundary (2026-09-13):** `minutes_into_day` deliberately refuses a `Z`-suffixed timestamp rather than guess a zone for it, so an event stored with one belonged to **no day at all**: absent from `blocks[]`, absent from the coverage arithmetic, and absent from `incomplete[]` too, while `create_event` returned `ok: true`. It also reached Google at the wrong instant, since the sync lifts the literal `HH:MM` out of the string. `interval.to_wall_clock(value, tz_name)` converts anything carrying an offset (`Z` or `+03:00`) into vault wall clock before `derive_interval` sees it, in both `create_event` and `update_event` — converts, not strips, because an offset names an instant and conversion is the only reading under which the ledger and Google agree. That can move the day, so the stored fragment date, the Google `date` and the `## Schedule` line all follow the converted start rather than the `date` argument that suggested one.
- **A block that cannot be placed is shown, not dropped (2026-09-13):** `_build_incomplete` skipped any event that had both timestamps, reading "has both" as "is a neighbouring day's fragment". Present-but-unreadable is the third case, and it fell through both filters — the Bug A shape again, one layer down. The test is now `_belongs_to_another_day`: only a timestamp that parses as a *different* calendar date is somebody else's fragment; anything else claiming this day surfaces under "Needs a time", listing the unreadable fields as missing, because from the day view's side "cannot read this" and "was never given this" are the same problem.
- **A row Mazkir authored survives a missing echo (2026-09-13):** `create_event` writes `source: "manual"` and then syncs to Google, which puts a `calendar_id` into `source_ids` — and from that moment `reconcile`'s deletion pass saw a calendar event. Any read where the calendar answered without returning it deleted the user's own block: gone from `/day` with `ok: true` in the tool result, and gone for good after one `GET /events/{date}`, which persists. The echo goes missing for reasons that have nothing to do with the user deleting anything — Google's insert is not read-your-writes consistent, `GOOGLE_CALENDAR_INCLUDE` need not name Mazkir's calendar, and the day window itself could miss it (see the next bullet). The ledger is the source of truth for temporal data and the calendar entry is a copy Mazkir pushed out, so a missing copy is not evidence the original never happened: `events_service.AUTHORED_SOURCES` (`manual`/`photo`, imported by `approval.py` rather than restated) is now never deletable by reconciliation. `delete_event` is still how a block goes away, and it removes both sides.
- **The calendar day window is a real day (2026-09-13):** `datetime(y, m, d, tzinfo=self.tz)` does not mean local midnight — a pytz zone attached directly hands back its *first historical* offset, LMT `+02:21` for Asia/Jerusalem — so `get_todays_events_with_status` queried 00:39 today to 00:39 tomorrow. An event living entirely in a day's first 39 minutes was never returned (and, per the bullet above, was then deleted for it), while one from the next day's first 39 minutes arrived under today's date. Both ends are `tz.localize`d midnights now, and the end is the *next date's* midnight rather than `start + 24h`, because Israel's October transition makes one day 25 hours long and the shorthand would cut the last hour off it.
...
- **A question the bot asks itself must travel with the answer (2026-09-12):** the gap prompt is a plain `ctx.reply` and never passes through `POST /message`, so the server has no record of it. Answering it with Telegram's reply-to works — `reply_to` becomes `[Replying to assistant: "…"]` in the prompt — but answering it by simply typing "Bar hopping" reached the agent as two words with no interval (`has_reply_to: false` in the log), and it asked for times the bot had named one message earlier. `src/state/open-question.ts` remembers the bot's own question per chat and `buildMessagePayload` presents it as the `reply_to` the user did not have to make, reusing the path the server already handles rather than adding a parallel notion of pending context. It is **one-shot** (consumed on read, so a stale interval cannot ride along on a later message) with a 10-minute TTL, is skipped when a real reply-to is present (the explicit reply wins, and the held question is dropped), and is armed **after** the send — the bot-wide `dropPerMessageHints` transformer clears these hints on every send, so arming first would be undone by the very reply that asked the question. That transformer (renamed from `dropSelectedDateHint`) now clears the open question too, so any other bot output ends it.
- **Only meaningful emoji go on calendar summaries (2026-09-12):** `_build_event` prefixed every plain event with `📅`, which said "this is a calendar event" inside a calendar. It is gone. The other prefixes stay because they carry information: `🎯` marks a habit, the priority emoji marks a task tier, and `✅` is **parsed back** as completion (`mark_event_complete` skips an already-marked summary; `get_todays_events` reads `summary.startswith('✅')` alongside Google's green `colorId`), so removing that one would silently break completion detection. Ownership of a Mazkir-made entry is recorded in `description` ("Managed by Mazkir"), never in the summary. Note this only affects newly written events — entries already in Google keep the prefix until renamed.
- **Payload provenance is on the span, because traces could not answer it (2026-09-12):** two rounds of debugging a lost gap question came down to grepping `vault-server.jsonl` by timestamp, because the 30 Phoenix spans covering the failing minute mentioned neither `reply_to` nor `selected_date` — `agent.handle_message` showed only that the model received a bare "Bar hopping", and 30 spans came to 90k characters. Both sides now stamp what they know. The bot sets `mazkir.payload.reply_to_source` (`telegram` / `open_question` / `none`), `.selected_date`, `.attachment_count` and `.has_forwarded_from` on `telegram.update`, from inside `buildMessagePayload` — the only place that knows whether the reply context was the user's own or the bot's own question handed back. The server sets `mazkir.payload.has_reply_to`, `.reply_to_from`, `.selected_date`, `.has_forwarded_from`, `.attachment_types` and `.text_length` on the `POST /message` span (`tracing_helpers.set_payload_provenance`). Read together they localise a lost hint to one side of the wire: both `none` means the bot never had it, a disagreement means it was dropped in transit. Absence is written as an **empty string rather than an omitted key**, on both sides, so a Phoenix filter can tell "nothing was sent" from "this span predates the attribute".
- **The typing indicator is not the bot saying something (2026-09-12):** `dropPerMessageHints` fires on every API call carrying a `chat_id`, and the NL handler's first act is `ctx.replyWithChatAction("typing")` — so it cleared both per-message hints roughly twenty lines before `buildMessagePayload` read them. The open question died on the turn that was answering it, and `selected_date` had been dying the same way, unnoticed, since Ship 4: a plain text message could never carry either. `NOT_A_MESSAGE` in `bot.ts` excludes `sendChatAction`; it is a denylist rather than an allowlist of real sends on purpose, because a send method added later and forgotten there still clears the hints (costing a re-ask), while the opposite default would let a stale hint ride along on an unrelated message (wrong data). The lesson for tests: `buildMessagePayload` in isolation passes either way, because the isolation is exactly what removes the typing indicator — so the ordering is asserted at the handler, driving the real transformer, in `tests/conversations/message.test.ts`.
- **An approved block the user authored keeps ✎ (2026-09-12):** approving is not the same as getting the times right, and a `manual`/`photo` block's times came from a sentence or a tapped guess — the likeliest to be wrong. `SELF_AUTHORED` in `day-rich.ts` mirrors `_HUMAN_SOURCES` in `approval.py`: the same two sources that make a block auto-approved are the ones whose times nothing upstream owns. Calendar- and timeline-sourced approved rows are deliberately excluded, since this ship writes nothing back to Google and a local edit would silently disagree with the source. ✎ travels alone there (no ✓/✕ — nothing left to confirm), the facet label moves beside the title so the third column can hold the button without the table growing a fourth, and `block-edit-rich.ts` drops the "& approve" half of its save label when the block is already approved. Note that `DailyBlock.source` in `packages/shared-types` had been missing `"manual"` and `"photo"` since Ship 4 while the server emitted both, so no bot code could branch on "did I make this myself" without TypeScript rejecting the comparison as impossible.
