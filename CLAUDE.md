...
- `PATCH /events/{date}/{event_id}` - Update a single persisted event; pins whichever of `name`/`location`/`start_time`/`end_time`/`activity` are present to `user_set`, so the next reconcile does not overwrite them from the source — `photos`/`assets` are never pinned this way, since they are preserved by other means (Ship 5). The body model forbids unknown fields (2026-09-13): it used to lack `start_time`/`end_time`, so the `/day` edit view's Save was silently dropped by pydantic, the route saved the event unchanged, and the bot toasted "✓ Saved" over an edit that never happened. A name or time edit on a block that mirrors a Google entry is pushed there too (`_push_edit_to_google`, returned as `calendar_sync`), under the same guards as the agent's `update_event`: only Mazkir's own calendar, never a cross-midnight span, and times padded to RFC 3339 seconds since the bot sends `…THH:MM`
- `POST /events/{date}/{event_id}/cancel` - Cancel the block's Google Calendar entry, keeping it restorable there (2026-09-16). Delete's reversible sibling, per design §6.3: `CalendarService.cancel_event` patches the entry to `status: "cancelled"`, and since `get_todays_events` lists without `showDeleted` it drops out of the merge and stops being re-suggested, while the block is stored `dismissed` so `/day` omits it and the record that it was planned survives. Same guards as delete — 409 with no Google entry or in another calendar, 503/502 when Google is unreachable or refuses — and one bot tap with no confirmation, because Google can restore it
- `DELETE /events/{date}/{event_id}` - Delete one block, and its Google Calendar entry when it has one (2026-09-15). Behind the `/day` edit view's delete button, which asks first (`evdel:ask` → `evdel:yes`); it had been drawn since Ship 5 and answered "Not wired up yet" to every tap. Resolves the block from the reconciled day, like `/state`. Refuses rather than half-deletes, writing nothing: 409 for a block rebuilt from a note checkbox, habit or location history (it would reappear on the next read — ✕ hides it instead) or an event in another calendar; 503/502 when Google is unreachable or refuses, since removing only Mazkir's copy of a synced block merges it straight back. "Cancel in calendar" is still inert
...
- **A proposal is Mazkir's guess, so it waits (2026-09-13):** "schedule these for later" names no times, and the agent used to add todos and ask when. `create_event` now takes `proposed: true` for times the agent chose: the row is still `source: "manual"`, but `resolve_state` returns `pending` for `proposed` before the human-source rule, so it arrives with ✓ ✕ ✎ instead of counting as something the user said. A proposal is not pushed to Google (`calendar_sync.reason: "proposed"`) and writes no `## Schedule` line, so a dismissed suggestion leaves nothing behind. **Approval is when it becomes real:** `set_event_state`'s `_settle_proposal` creates the Google entry, stores its `calendar_id`, and writes the Schedule line, using the block's current times — which already carry any ✎ adjustment, because the edit view saves before it approves. A failed sync is reported as `calendar_sync` and never blocks the approval. "Approve all" goes through the same function. `day-rich.ts` gives a still-ahead pending self-authored block its buttons, since a proposal is in the future by nature and would otherwise be unadjustable until its time had passed.
...
- **Google's `dateTime` needs seconds, and `update_event` now guarantees them (2026-09-15):** the agent's `update_event` tool stores times the way it receives them, and `_normalize_time` only pads a bare `HH:MM` — a full `2026-09-14T23:45` passes through unchanged, and so does `interval.normalize_time`. That value went straight into the patch body, and Google answered with an unexplained 400 "Bad Request": moving "Washing machine" to 23:45–23:55 changed Mazkir's copy while Google stayed on 23:55–00:25, and the reply could only say the sync failed. The ✎ route had already padded its own times with a local `_rfc3339`, which is why nudge-pad edits synced and spoken ones did not. `calendar_service._with_seconds` now pads minute-precision timestamps (keeping any offset) inside `CalendarService.update_event` itself, the one place every edit reaches Google, so no caller can reintroduce it; it is idempotent, so the route's own padding is harmless. The ledger keeps its mixed precision, since `fromisoformat` reads both.
...
- **A reply to a rich message has no `.text` (2026-09-15):** agent answers and the `/day`, `/tasks`, `/habits` and `/goals` views are sent as rich messages, which carry `rich_message.blocks` and no `text`. `buildMessagePayload` read only `reply_to_message.text`, so every Telegram reply to an agent answer arrived with no context — "record it between 23:45 and 23:55 instead", replying to a washing-machine message, logged `reply_to_source: "none"` and the agent had to ask which block. It had been that way since agent replies became rich on 2026-06-19; the only assistant replies whose context ever reached the server were plain-text bot messages (the gap prompts), which is why it read as a regression. `src/bot-utils/rich-text.ts`'s `repliedToText(msg)` now resolves the answered text in order: Telegram's `quote.text` (the part the user selected), `text`, `caption`, then the rich blocks flattened to plain text. The flattener recurses through lists, quotes, details and tables, keeps only captions from media, contributes nothing for buttons, and returns `""` for an unknown block type so a Bot API addition shortens the context rather than dropping the reply.
...
- **`agent-turns.jsonl` is memory, not just audit (Ship 3):** `MemoryService.assemble_context` reads this chat's records for today and attaches each turn's calls as `[Record of your previous reply — tools that actually ran, as <skill>: …]` — at the start of the **user** message after that reply, and for the newest reply on the incoming message via `ConversationContext.trailing_trace`. **Never inside the assistant's own text (2026-09-13):** it used to be appended to the reply as `[Tools I called this turn …]`, the model learned from its own history that replies end that way, and five turns between 09-10 and 09-13 wrote a forged record instead of calling a tool — habits "created", blocks "logged", chores "added", none of it real, and PRs #24/#25 were opened against bugs that did not exist because the reports were built on those claims. Nothing detects or strips such a record: the raw model output for every call is already in Phoenix (`llm.output_messages` on the Anthropic spans), which is where these turns were diagnosed. The agent therefore reads what it actually did instead of inferring it from its *current* skill's tool list — the failure that made it deny its own writes on 2026-08-20. `services/turn_trace.py` owns reading, rendering and the join; it never raises, so a torn log line costs one trace rather than the turn. The join matches on exact `user_text` walking from the tail, and **a mismatch attaches nothing** — a wrong trace would be a worse version of the bug, arriving dressed as evidence. Deleting `data/logs/` now costs the agent this memory; the prompt invariant ("never deny a past action without checking") is the safety net that holds without it.
