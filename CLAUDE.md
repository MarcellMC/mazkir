...
- `POST /daily/{date}/approve-all` - Approve all pending blocks for a date (Ship 5): `{selected_date}` → processes blocks that have `elapsed`, auto-skips still-ahead ones, updates their state to `approved`, stamps them with the block's own date (not today)
- `POST /daily/{date}/gaps/fill` - Fill a gap with a proposed activity (Ship 5): `{gap_index, activity_name?}` → creates or updates the event for that gap interval
...
- `POST /events/{date}/{id}/state` - Update event approval state (Ship 5): `{state: "approved"|"pending"|"dismissed"}` → updates the persisted state and returns the updated event
- `PATCH /events/{date}/{event_id}` - Update a single persisted event; now pins fields specified in the request to `user_set` (Ship 5)
...
- **A `date` path param is typed, never a raw string (Ship 5):** every `date` in `api/routes/events.py` is declared `date: date_type`, so FastAPI rejects anything that is not a real calendar date with a 422 before a handler runs. This is load-bearing rather than tidy: `EventsService._file_path` builds `self.events_path / f"{date}.json"`, so an unvalidated string lets the request path choose a filesystem **write** target via `save_events`. `routes/daily.py` records having made the same change after `?date=../../../../etc/hosts` read an arbitrary file off disk. Note that `_set_state_for_approve_all` calls `set_event_state` as a direct Python call rather than over HTTP, so FastAPI's coercion never runs there and the seam converts the string itself — a test pins that, because every other approve-all test mocks the seam and would not catch its absence.
